    ...
    if(offerModelStore.offer.id) {
      ...
      offerApi.patch(offer_id, offerModelStore.offer)
        .then(offer => {
          ...
          // childOfferStore.fetchOfferStats(page);
          childOfferStore.fetchChildOfferStats(page,childOfferStore.campaign_id);
          offerModelStore.closeModal();
          ...
          { offerModelStore.offer &&
            <RctCollapsibleCard  heading={ offerModelStore.offer['name'] || 'Offer Name' }>
              <Form onSubmit={ this.handleSubmit.bind(this) } className="modal-form">
                ...
                <FormGroup className="col-sm-12 col-md-12 col-xl-12">
                  <h4>Payout Sum</h4>
                  <Input type="number" min="0.0" step="0.01" name="payout" disabled  placeholder="Offer Payout" value={ offerModelStore.offer['payout_sum'] } onChange={ this.handleChange.bind(this) } required />
                </FormGroup>
                {

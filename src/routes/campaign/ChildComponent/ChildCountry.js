...
import childCountryStore from 'Storage/childCountryStore';
...
class ChildCountry extends Component {
  ...
  constructor(props) {
    ...
    this.childoffer = '';
    this.state = {
      page:childCountryStore.current_page,
      search_value : ''
    };
  ...
  componentDidMount() {
    ...
    // childCountryStore.fetchChildCountryStats(page,campaign_id,this.search_value);
    // this.setState({page:childCountryStore.current_page})
    const campaign_id = this.props.match.params.campaign_id;
    if(utcStore.utc == "utc-8"){
      ...
      childCountryStore.setDateRange(start_timestamp_utc, end_timestamp_utc);
      childCountryStore.fetchChildCountryStats(childCountryStore.current_page,campaign_id);
    }
    else{
      ...
      childCountryStore.setDateRange(start_timestamp, end_timestamp);
      childCountryStore.fetchChildCountryStats(childCountryStore.current_page,campaign_id);
    }
  }
  ...
  handleSearchSubmit(e){
    ...
    const campaign_id = this.props.match.params.campaign_id;
    childCountryStore.searchPage(childCountryStore.current_page,campaign_id,search_value);
    if(search_value==''){
      childCountryStore.fetchChildCountryStats(childCountryStore.current_page,campaign_id,this.search_value);
    }
  ...
  render() {
    ...
    const { loaded, error } = childCountryStore;
        ...
        {localStorage.getItem('user_id') &&
          <React.Fragment>
            <div className="about-wrapper campaign-child">
              <Helmet>
                <title>{AppConfig.brandName} - Country</title>
              </Helmet>
                ...
                <RctCollapsibleCard heading={
                  <div className="row no-gutters row-eq-height align-items-center">
                    ...
                    {/* <div className="child-chip">
                      ...
                      <Chip label="Country" className="chip-text" />
                    </div> */}
                    <div className="child-chip">
                      ...
                      <Chip label="Country" className="chip-text chip-border-radius" component="a" href={ `/campaign/${campaign_id}/${campaign_name}/country` } clickable />
                    </div>
                  </div>
                } fullBlock>
                   ...
                   <div className="table-responsive table-topfix" style={styleObj}>
                     <Table>
                       ...
                       <TableBody classes={{ root:classes.tableBodyNone }}>
                         ...
                         { error && <p>{ error }</p> }
                         {childCountryStore.Countries.countries &&
                           childCountryStore.Countries.countries.map(country => (
                             <React.Fragment>
                              <TableRow hover key={country.id}>
                                <TableCell numeric className="text-left paddingTd pl-20">{country.country_name_en}</TableCell>
                                <TableCell numeric className="text-left paddingTd">{country.visit}</TableCell>
                                <TableCell numeric className="text-left paddingTd">{country.click}</TableCell>
                                <TableCell numeric className="text-left paddingTd">{country.ctr}</TableCell>
                              </TableRow>
                             </React.Fragment>
                           ))
                         }
                         {
                           !childCountryStore.Countries.countries &&
                            childCountryStore.Countries.map(country => (
                            <React.Fragment>
                             <TableRow hover key={country.id}>
                               <TableCell numeric className="text-left paddingTd pl-20">{country.country_name_en}</TableCell>
                               <TableCell numeric className="text-left paddingTd">{country.visit}</TableCell>
                  ...
                  {
                    childCountryStore.searched &&
                    <React.Fragment>
                      <div class="text-right page-action">
                        <Pagination
                          current={ childCountryStore.current_page }
                          total={ childCountryStore.total_page }
                          goToPage={ (page) => childCountryStore.fetchOfferStats(page) }
                        >
                        </Pagination>
                      </div>
                    </React.Fragment>
